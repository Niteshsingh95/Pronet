...
function EditProfile() {
  ...
  let [fontendProfileImage, setFontendProfileImage] = useState(userData.profileImage || dp);
  let [backendProfileImage, setBackendProfileImage] = useState(null);
  let [fontendCoverImage, setFontendCoverImage] = useState(userData.coverImage || null);
  ...
  function handleProfileImage(e) {
    ...
    if (file) {
      setFontendProfileImage(URL.createObjectURL(file));
  ...
  function handleCoverImage(e) {
    let file = e.target.files[0];
    if (file) {
      setFontendCoverImage(URL.createObjectURL(file));
    ...
    <div className=" w-full h-[100vh] fixed top-0  z-[100] flex items-center justify-center ">
      ...
      <div className="w-[90%] max-w-[500px] h-[600px] bg-white relative  overflow-auto z-[200] shadow-lg rounded-lg p-[10px] ">
        ...
        <div className="w-full h-[150px] bg-gray-500 rounded-lg mt-[40px] overflow-hidden" onClick={()=> coverImage.current.click()}>
          <img src={fontendCoverImage} alt="" className="w-full " />
          <FaCamera className="absolute right-[20px] top-[60px] w-[25px] h-[25px] text-white cursor-pointer" />
        </div>
        <div className="w-[80px] h-[80px] rounded-full overflow-hidden absolute top-[165px] ml-[10px] " onClick={()=> profileImage.current.click()}>
          <img src={fontendProfileImage} alt="dp" className="w-full h-full" />
        </div>
        <div className="w-[20px] h-[20px] bg-[#17c1ff] absolute top-[215px] left-[80px] rounded-full flex justify-center items-center">
        ...
        <div className="w-full flex flex-col items-center justify-center gap-[20px] mt-[60px]">
          ...
          <input type="text" placeholder="headline"  className="w-full h-[50px] outline-none border-gray-600 px-[10px] py-[5px] text-[18px] border-2 rounded-lg"
          ...
          <input type="text" placeholder="gender (male/famale/other)" className="w-full h-[50px] outline-none border-gray-600 px-[10px] py-[5px] text-[18px] border-2 rounded-lg"  value={gender} onChange={(e)=>setGender(e.target.value)} />

          <div className="w-full p-[10px] border-gray-600 flex flex-col gap-[10px] border-2 rounded-lg">
            <h1 className="text-[19px] font-semibold">Skill</h1>
            {skills && <div className="flex flex-col gap-[10px]">
              {skills.map((skill, index) => (
                <div key={index} className="w-full h-[40px] border-[1px] border-gray-600 bg-gray-200 rounded-lg p-[10px] flex justify-between items-center"> <span>{skill}</span> <RxCross1 className="w-[20px] h-[20px] text-gray-700 font-bold cursor-pointer" onClick={()=>removeSkill(skill)}/></div>
              ))}
              </div>}
            ...
            {education && <div className="flex flex-col gap-[10px]">
              {education.map((edu, index) => (
                <div key={index} className="w-full  border-[1px] border-gray-600 bg-gray-200 rounded-lg p-[10px] flex justify-between items-center"> <div>
                 ...
                 <RxCross1 className="w-[20px] h-[20px] text-gray-700 font-bold cursor-pointer" onClick={()=>removeEducation(edu)}/></div>
              ))}
              </div>}
              <div  className="flex flex-col gap-[10px] items-start" >
            ...
            {experience && <div className="flex flex-col gap-[10px]">
              {experience.map((exp, index) => (
                <div key={index} className="w-full  border-[1px] border-gray-600 bg-gray-200 rounded-lg p-[10px] flex justify-between items-center"> <div>
                 ...
                 <RxCross1 className="w-[20px] h-[20px] text-gray-700 font-bold cursor-pointer" onClick={()=>removeExperience(exp)}/></div>
              ))}
              </div>}
              <div  className="flex flex-col gap-[10px] items-start" >
          ...
          <button className='w-[100%] h-[50px] rounded-full bg-[#0898c4] mt-[40px] text-white' disabled={saving} onClick={()=>handleSaveProfile()}>{saving?"saving...":"Save Profile"}</button>
